...
describe("Encryption", () => {
    it("should use `magickey` to encrypt `articles`", () => {
        ...
        expect(sc.encryptToQRCodeAlphanumeric("articles")).toBe(
            "3BHNNR45XZH8PU",
        );
    });
});
...
describe("Decryption", () => {
    it("should use `magickey` to decrypt ciphers to `articles`", () => {
        ...
        expect(
            Buffer.from(sc.decryptURLComponent("2E87Wx52-Tvo") || []).toString(
                "utf8",
            ),
        ).toBe("articles");
        expect(
            Buffer.from(sc.decryptQRCodeAlphanumeric("3BHNNR45XZH8PU") || [])
                .toString("utf8"),
        ).toBe("articles");
    });
});
